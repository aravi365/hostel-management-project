...
<table class="table table-striped table-hover">
  <thead id="th">
	...
	<?php
	  ...
	  mysql_select_db("outpass");
	   $rows= mysql_query("SELECT * FROM requestwarden where status = 0  and type = 1");
 ...
 <td>        <?php if($hod==1) { ?> <form action ="wsapproveoutpass.php" method ="post" ><input type ="hidden" name ="opid" value ="<?php echo $no; ?>"> 
     <input  name ="submit" type="submit" value="Approve"> </form> <?php } ?> </td>  
	  <td>  <form action ="wsrejectoutpass.php" method ="post" > 
	  <input type ="hidden" name ="opid" value ="<?php echo $no; ?>"> 
	  <?php if($hod== 2) { ?> <input  name ="submit" type="submit" value="Reject"> <?php } ?> </td> 
	  </form>
    </tr>
	  <?php } ?>
